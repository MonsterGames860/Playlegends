...
function roomPublicInfo(room) {
    return {
        code:        room.code,
        name:        room.name,
        host:        room.creatorName,   // client room.host olarak okur
...
io.on('connection', (socket) => {
    ...
    socket.on('lobby:close', ({ code }) => {
        ...
        if (!isCreator && !hasOp) {
            return socket.emit('lobby:close:response', {
                success: false,
                error:   'Sadece sunucu sahibi veya OP kapatabilir. / Only creator or OP can close.'
            });
        }
    ...
    socket.on('disconnect', () => {
        ...
        // Kurucu ayrıldıysa creatorSocketId'yi temizle — oda KAPANMAZ, yaşamaya devam eder
        if (room.creatorSocketId === socket.id) {
            room.creatorSocketId = null;
            console.log(`[SERVER] Kurucu ayrıldı ama oda açık kalmaya devam ediyor: ${code}`);
        }
